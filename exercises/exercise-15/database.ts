...
type DatabaseQuery<T> = SimpleQuery<T> | SetQuery<T> | TextQuery;
...
export class Database<T extends DatabaseRecord> {
    ...
    async find(query: DatabaseQuery<T>, options?: QueryOptions<T>): Promise<Partial<T>[]> {
    ...
    async delete(query: DatabaseQuery<T>) {
        const rowsToDelete = await this.find(query);
        const idsRegexes = rowsToDelete.reduce((res: RegExp[], row) => {
            return row._id !== undefined 
                ? [ ...res, new RegExp(`E(?={"_id":${row._id})`, 'g') ] 
                : res
        }, []);
    ...
    async insert(record: T) {
        const preparedEntry = `E${JSON.stringify(record)}\n`;
        try {
            await fs.appendFile(this.filename, preparedEntry);
        } catch (error) {
            console.error('Error occurred:', error);
        }
    }
